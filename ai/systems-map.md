...
| Bar activity routing | Assign the first three stable row-major bars to cocktail mixing, beer pong and Split the G through one pure resolver, fall back to cocktails for later bars, preserve the activity through transition, then resolve its registered factory | `BarActivityKind`, `BarActivityAssignment`, `BarMinigameCatalog`, `BuildingLot`, `BarEntrance`, `GameSessionState` | Implemented |
