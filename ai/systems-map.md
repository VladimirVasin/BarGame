...
| City Blender misc catalog | CityMisc `4.10.0`: `82` kinds / `122` assemblies / `259` passive role meshes / `46,546` triangles; plans retain placement, collision and dynamics. | `CityMiscAssetProvider{,.LateCatalog}`, `CityMiscAssetSetup` | Current |
...
| Hero inventory | Shared modal inventory owns item actions and separate clothing equipment status; stacks/equipment persist within the session. | `Inventory{Types,State,MenuModel,Controller,View}`, `GameSessionState` | Current |
...
| Mother's house interior | Two bedrooms and a passive bathroom in a supported rear wing; shared windows, walkable stairs and five height-aware cameras. | `MothersHouseInterior*`, `MothersHouseWindowLayout`, `HomeFixedCameraController` | Current |
...
| The mother in her chair | Silent sitter and chair share measured runner contact on the rug; her hips follow the tilted cushion. Voice, interaction and expression driving remain unwired. | `MothersHouseMother{Plan,Presentation,Factory,Provider,AssetSetup}`, `MothersHouseRockingChairMotion` | Current |
...
| Begotten sound perception | The print's optical track tears the world bus against its own mask throughout the ramp, rides a fixed projector fader, and adds surface and apparatus on a 24-per-second grid from the picture's own ramp. | `BegottenAudioRules`, `BegottenAudioDriver`, `OpticalProcessor.h` | Current |
...
| Wearable scarf | Bounded wind/motion and simple hero-body contacts; external objects are ignored. Mouth access, visibility and mirror share the result. | `PlayerScarf*`, `PlayerFactory` | Current |
| Alpine Village cold hero | Self-hug/rubs persist through running; equipped scarf halves shiver amplitude while gait, breath and protective actions keep priority. | `Player3DCharacterPresentation.Cold`, `PlayerCold{PresentationModel,BreathEffect}` | Current |
...
| Home bathroom scenes | Shower hero exits fully offscreen before redressing; the camera retraces entry before the final curtain close. Cleanup restores rig and input. | `HomeBathroomSceneInteraction`, `HomeShower{Interaction,CameraPath}`, `HomeTeethBrushingInteraction` | Current |
...
| Shower water presentation | Visible jets follow the tilted nozzle; residual drops/ripples share a tray landing point. Valve turns and landings have local sound. | `HomeShower{Interaction,TrayWater}`, `HomeSoundscape` | Current |
