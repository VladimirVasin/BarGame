...
| City and Home street pedestrians | One deterministic radius-safe sidewalk/zebra graph feeding at most two player-relative actor slots (one at night), each filled by a seeded pick from an ordered five-archetype catalog whose pool exceeds the active cap. One design is airborne and one carries a single bounded shadowless Spot; both are declared per archetype rather than allowed implicitly. Fog-band spawn and recycle never depend on camera direction or frustum. | `Runtime/City/NPC`, `CityGameRoot`, `HomeInteriorRoot`, `RoadWalkableArea`, `Resources/Pedestrians/{CityPedestrian3D,ChairCarrierPedestrian3D,KettleHatPedestrian3D,LongArmPedestrian3D}` | Current |
...
| Route 01 passenger MVP | Board and alight at either fully open passenger door into fixed window seat `07`, with authored `BusBoardEnter`/`BusRideLoop`/`BusAlightExit` and a seat-following camera whose horizon ignores suspension roll. **Gap:** fare, destination choice, NPC passengers, persistence and live tracking are out of scope. | `CityBusRide{Plan,Controller}`, `CityBus{Actor,Director}`, `PlayerAnimatedInteractionController`, `PlayerCameraFollow`, `CityStreetSurfacePlan` | Partial |
...
blueprint ID + seed -> immutable blueprint -> validated sparse layout
  ...
  -> street surface plan -> Road v2 carriageway + sidewalks + Road v2.1 aprons
     -> pedestrian graph -> two player-relative walker slots
