...
| Eastern mainland view | Closed post, shared relief/swale, sparse trees, clear paths, local lights; distant road/skyline. | `CityEastExit*`, `CityEast{Relief,Swale,Tree}Plan`, `CityEastDistanceWorldBuilder` | Current |
| Eastern verge litter | 36 passive Blender variants across five width bands; shared rigid meshes, grounded solids and clear walking routes. | `CityLitterCatalog`, `CityEastLitter{Plan,WorldBuilder}` | Current |
...
| Authored NPC outfits | Validated garment slots; body/face/hair/glasses independent. One outfit each; UI/save Deferred. | `NpcWardrobe`, `CanneryWomanWardrobe` | Current |
...
| Ferryman car and passenger seat | Doors, attachment, camera, audio, area arrival; callouts outline radio knobs and glovebox handle. | `LastRouteCar{Plan,Factory,Doors,Suspension}`, `LastRouteCarDashboard`, `LastRouteRadioAffordance` | Current |
