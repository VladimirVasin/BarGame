...
| Player dynamic shadow | Cast one camera-independent alpha-clipped realtime silhouette in City and BarInterior; select the existing full-body view relative to the main directional light, face the shadow-only card toward that light and follow whole-puppet bob/sway without enabling shadows on practical lights | `PlayerDynamicShadow`, `PlayerShadowResources`, `PlayerSpriteShadowCaster.shader`, main directional light | Implemented |
