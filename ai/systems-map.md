...
| Mother's house interior | Separate two-storey house with furnished bedrooms, runtime stair collision and height-aware fixed cameras; the ground shot pans and re-lenses to keep the hero half the frame. | `MothersHouseInterior*`, `HomeFixedCameraController` | Current |
...
| Mirror brushing | First-person X/Y brushing uses the real reflection; the hero opens the faucet before brushing and closes it after spit or cancellation. | `HomeTeethBrushing*`, `HomeBrushingFirstPersonView`, `HomeSinkFaucet` | Current |
| Toilet choice and bowl camera | Two-option menu; large option moves the camera below bowl water for a `2.5/3/2.5 s` sequence without relief. | `HomeToiletChoiceInteraction`, `HomeToiletPlunge{Interaction,Timeline}` | Current |
