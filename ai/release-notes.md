...
- **Табличка.** Как камень встал, камера обходит его спереди: на лицевой
  ...
  настоящие, отчеканены прямо по латуни. Имя — «Безымянный», годы — «Годы неизвестны»,
  а третью строку пишешь сам с клавиатуры, не длиннее восьми слов —
  счётчик оставшихся прямо под полем. `Enter` — вырезать, `Esc` —
  оставить пустой. Написанное не переписывается; подойдёшь к готовой
  могиле потом — `E` прочитает табличку снова.
