...
| `scene` | loaded/ready plus transition requested, rejected, fallback, completed or failed; `composition_frames` records frames, `path` (`area`/`door`) and `AdvanceFrame` CPU versus wall time; a door `transition_completed` adds `composition_frames`, its `duration_ms` including the build |
