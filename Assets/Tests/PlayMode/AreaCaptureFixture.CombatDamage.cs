    ...
    public sealed partial class AreaCaptureFixture
    {
        ...
        private IEnumerator CaptureCombatDamage(CombatTestRoot root, Camera camera)
        {
            foreach (bool hurtHero in new[] { false, true })
            {
                ...
                CaptureDamageFrame(root, camera, "damage-" + subject + "-" + Mathf.RoundToInt(S.MaxHealth));
                float previousInjury = 0f;
                for (int strike = 1; strike <= HitsToDefeat; strike++)
                {
                    ...
                    for (int step = 0; step < ContactTicks(120f) + 24 && target.State.Health == before; step++)
                        root.Tick(1f / 120f);
                    Assert.That(target.State.Health, Is.EqualTo(before - S.Damage));
