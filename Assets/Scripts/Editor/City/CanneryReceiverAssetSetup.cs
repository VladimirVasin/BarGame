...
{
    /// <summary>Explicit authoring/import and read-only packaging validation for the one receiver.</summary>
    public static class CanneryReceiverAssetSetup
    {
        public const string Folder = "Assets/Resources/City/Cannery/Receiver/";
        public const string ModelPath = Folder + "CanneryReceiver.fbx";
        public const string ActionPath = Folder + "CanneryReceiverActions.fbx";
        public const string PrefabPath = Folder + "CanneryReceiverActor.prefab";
        private static readonly string[] RequiredActions = { "CanneryReceiverIdle", "CanneryReceiverWalk",
            "CanneryReceiverWork", "CanneryReceiverListen", "CanneryReceiverBreak" };
        private static readonly string[] RequiredJoints = { "root", "pelvis", "spine", "chest", "neck", "head",
            ...
            "SOCKET_Grip.L", "SOCKET_Grip.R", "SOCKET_Mouth", "ANCHOR_Glasses", "ANCHOR_GlassesFinger",
            "ANCHOR_HandFingers.L", "ANCHOR_HandFingers.R", "ANCHOR_HandPalm.L", "ANCHOR_HandPalm.R" };

        [MenuItem("Bar Promenade/City Cannery/Rebuild Receiver")]
        ...
        public static void BuildOrThrow()
        {
            ...
            if (avatar == null || !avatar.isValid)
                throw new InvalidOperationException("The cannery receiver's own broad rig requires its generated Generic Avatar.");
            ConfigureModel(ActionPath, true, avatar);
            ConfigureTexture(Folder + "CanneryReceiverAtlas.png");
            ConfigureTexture(Folder + "CanneryReceiverFaceAtlas.png");
            ...
            AnimationClip Own(string name) => own.Single(clip => clip.name == "CanneryReceiver" + name);
            ...
            GameObject root = new GameObject("Cannery Receiver");
            try
            {
                ...
                NpcWardrobe.GarmentBinding[] outfit = manifest.outfit.parts.SelectMany(part =>
                    part.renderers.Select(name => new NpcWardrobe.GarmentBinding(part.slot,
                ...
                Color Tint(string name)
                {
                    ...
                    // Blender records linear color; MaterialPropertyBlock.SetColor takes sRGB.
                ...
                Transform Find(string name) => CityPedestrianHandProps.FindSocket(model.transform, name)
                    ?? throw new InvalidOperationException("Missing cannery receiver joint " + name);
                VillageResidentPresentation motion = root.AddComponent<VillageResidentPresentation>();
                motion.ConfigureAuthoredActor("CanneryReceiver", animator, model.transform, Find("SOCKET_Grip.R"),
        ...
        public static void ValidateOrThrow()
        {
            ...
            if (prefab == null || prefab.GetComponent<CanneryReceiverPresentation>() == null)
                throw new InvalidOperationException("Missing authored cannery receiver prefab.");
            var receiver = prefab.GetComponent<CanneryReceiverPresentation>();
            if (receiver.Motion == null || receiver.FaceRenderer == null || receiver.GlassesRoot == null || receiver.Motion.Animator == null ||
                receiver.Motion.Animator.applyRootMotion || receiver.Motion.Animator.runtimeAnimatorController != null)
                throw new InvalidOperationException("Cannery receiver lost his shared bone-only presentation bindings.");
            if (receiver.Wardrobe == null || receiver.Wardrobe.CurrentOutfitId != CanneryReceiverPresentation.WorkwearId)
                throw new InvalidOperationException("Cannery receiver lost the explicit current outfit binding.");
            receiver.Wardrobe.ValidateBindings();
            if (receiver.Wardrobe.Owns(receiver.FaceRenderer))
                throw new InvalidOperationException("The painted face cannot belong to a clothing outfit.");
            Manifest source = ReadManifest();
            foreach (NpcWardrobe.GarmentBinding garment in receiver.Wardrobe.Garments)
            ...
            foreach (string joint in RequiredJoints)
                if (CityPedestrianHandProps.FindSocket(receiver.Motion.ModelRoot, joint) == null)
                    throw new InvalidOperationException("Cannery receiver lost joint/socket " + joint);
            Texture2D face = AssetDatabase.LoadAssetAtPath<Texture2D>(Folder + "CanneryReceiverFaceAtlas.png");
            if (face == null || face.width != 512 || face.height != 256 || face.filterMode != FilterMode.Point)
                throw new InvalidOperationException("Cannery receiver face must retain the shared 8 x 4 painted atlas contract.");
            ...
            if (clips.Length != RequiredActions.Length || RequiredActions.Any(name => !clips.Any(clip => clip.name == name)))
                throw new InvalidOperationException("Cannery receiver must retain his five authored actions.");
            foreach (AnimationClip clip in clips)
                if (clip.length <= 0f || !clip.isLooping || AnimationUtility.GetAnimationEvents(clip).Length != 0 ||
                    ...
                    throw new InvalidOperationException("Cannery receiver's actions must be looping bone-only curves: " + clip.name);
            Renderer[] glasses = prefab.GetComponentsInChildren<Renderer>(true)
                .Where(renderer => renderer.name.StartsWith("GEO_Glasses", StringComparison.Ordinal)).ToArray();
            if (glasses.Length < 3 || glasses.Any(receiver.Wardrobe.Owns) ||
                !receiver.GlassesRoot.IsChildOf(receiver.Motion.Head))
                throw new InvalidOperationException("The receiver's modeled frame and temples must remain permanent head-bound geometry.");
        ...
        private static void ConfigureModel(string path, bool animation, Avatar avatar)
        {
            ...
            if (importer == null) throw new InvalidOperationException("Missing generated cannery receiver model " + path);
        ...
        private static void ConfigureTexture(string path)
        {
            ...
            if (importer == null) throw new InvalidOperationException("Missing painted cannery receiver texture " + path);
        ...
        {
            Manifest manifest = JsonUtility.FromJson<Manifest>(File.ReadAllText(Folder + "CanneryReceiver.json"));
            if (manifest == null || manifest.triangle_count < 6000 || manifest.triangle_count > 8000 ||
                manifest.height_scale <= 0f || Mathf.Abs(manifest.height_m - 1.96f) > .001f || manifest.parts == null)
                throw new InvalidOperationException("Cannery receiver source violates the revised 6–8 thousand triangle contract.");
            if (manifest.outfit == null || manifest.outfit.id != CanneryReceiverPresentation.WorkwearId ||
                string.IsNullOrEmpty(manifest.outfit.atlas) || manifest.outfit.parts == null || manifest.outfit.parts.Length == 0)
                throw new InvalidOperationException("Cannery receiver source needs its one explicit outfit contract.");
        ...
        private static void ValidateBody(GameObject actor, Manifest manifest)
        {
            ...
            if (renderers.Length != manifest.mesh_count)
                throw new InvalidOperationException("Cannery receiver imported renderer count differs from his source.");
            ...
            foreach (Renderer renderer in renderers)
            {
                ...
                if (mesh == null || mesh.uv.Length != mesh.vertexCount)
                    throw new InvalidOperationException("Cannery receiver lost authored geometry/UVs: " + renderer.name);
            ...
            if (Mathf.Abs(error.x) > .015f || Mathf.Abs(error.y) > .015f || Mathf.Abs(error.z) > .015f ||
                ...
                throw new InvalidOperationException($"Cannery receiver imported metre/triangle mismatch: {high - low:F4} / {expected:F4}, bottom {low.y:F4}, triangles {triangles}/{manifest.triangle_count}.");
        }

        [Serializable] private sealed class Manifest
        {
            public int mesh_count, triangle_count; public float height_scale, height_m;
