#!/usr/bin/env python3
"""Validate and pack the authored player balcony-smoking atlas.
...
The builder never creates or substitutes source art. Pillow is its only
dependency.
"""
...
except ImportError as exc:  # pragma: no cover - runs before this module loads.
    raise SystemExit(
...
class SmokingAtlasError(RuntimeError):
...
def make_fit_plan(source_size: tuple[int, int]) -> FitPlan:
    ...
    if source_width * CELL_HEIGHT >= source_height * CELL_WIDTH:
        ...
        crop_width = source_height * CELL_WIDTH / CELL_HEIGHT
        ...
        crop_height = source_width * CELL_HEIGHT / CELL_WIDTH
        crop_left = 0.0
...
def validate_atlas(
    ...
    if atlas.size != (ATLAS_WIDTH, ATLAS_HEIGHT):
        raise SmokingAtlasError(
            f"Atlas must be 1024x768, got {atlas.width}x{atlas.height}."
        )
...
def main() -> int:
    ...
    try:
        run(args)
    except SmokingAtlasError as exc:
