...
behind its west sofa to a narrow upper corridor and three furnished
rooms: a shared bathroom in the extended west wing, the parents' bedroom to the north, still slept in, and the hero's
...
UPPER_DOOR_CENTERS_Z = (-1.85, 1.10)
WING_REAR_WALL = 4.90
BATHROOM_X = (-4.88, -1.83)
BATHROOM_Z = (1.98, 4.78)
BATHROOM_PARTITION_Z = 1.90
BATHROOM_DOOR_X = -2.505
BATHROOM_DOOR_WIDTH = 1.20
...
def window_records() -> list[dict]:
    ...
    for source in WINDOW_LAYOUT["windows"]:
        ...
        row["wall_plane"] = row.get("wall_plane", {"north": 4.0, "south": -4.0, "east": 5.0, "west": -5.0}[wall])
        normal_index = 2 if wall in ("north", "south") else 0
        position[normal_index] = row["wall_plane"] - (1 if wall in ("north", "east") else -1) * 0.18
...
def window_runtime_source() -> str:
    ...
    for row in WINDOWS:
        ...
        rows.append(
            ...
            f'                "{row["frame_part"]}", "{row["glass_part"]}", {number(row["wall_plane"])}, {str(row.get("frosted", False)).lower()}),')
...
CORRIDOR_SHELF_Z = (-1.10, 0.24)
CORRIDOR_SHELF_HEIGHT = 1.94
# Retain this established part's semantic identity while moving the cleaning
# set off the narrow stair landing into the bathroom's south-west pocket.
CORRIDOR_PAIL_X = (-3.95, -3.55)
CORRIDOR_PAIL_Z = (2.08, 2.42)
...
CORRIDOR_RUNNER_Z = (-2.55, 1.45)
CORRIDOR_PEG_Z = (-0.70, 0.20)
...
CORRIDOR_CHEST_X = (-1.55, -0.45)
CORRIDOR_CHEST_Z = (2.05, 2.75)
...
ANCHORS_UNITY = {
    ...
    "ANCHOR_FireLight": (0.0, 0.78, 3.50),
    "ANCHOR_Tabletop": (0.0, TABLE_HEIGHT, 0.0),
    "ANCHOR_TeapotDock": (0.18, TABLE_HEIGHT + 0.03, 0.05),
    "ANCHOR_FloorLampLight": (-1.95, 1.50, -1.62),
    "ANCHOR_UpperCorridorLampLight": (-2.50, 5.62, -0.60),
    "ANCHOR_BathroomCamera": (-2.40, 5.40, 0.70),
    "ANCHOR_BathroomCameraTarget": (-3.20, 4.40, 3.50),
    "ANCHOR_BathroomLampLight": (-2.13, 5.25, 3.55),
    "ANCHOR_BathroomColdWindowLight": (-3.55, 5.00, 4.62),
}
...
ANCHOR_ROLES = {
    ...
    "ANCHOR_UpperCorridorLampLight": "upper_corridor_lamp_light",
    "ANCHOR_BathroomCamera": "bathroom_camera",
    "ANCHOR_BathroomCameraTarget": "bathroom_camera_target",
    "ANCHOR_BathroomLampLight": "bathroom_lamp_light",
    "ANCHOR_BathroomColdWindowLight": "bathroom_window_light",
}
DEFAULT_BLEND = (
...
class Part:
    ...
    flame_uv: list[tuple[float, float]] = field(default_factory=list)
    flame_colors: list[tuple[float, float, float, float]] = field(default_factory=list)
    ...
    return merge((
        ...
        kit.sweep(
            (
                (half_x, -half_z),
                (half_x, half_z),
                (UPPER_PARTITION_X - .08, half_z),
                (UPPER_PARTITION_X - .08, WING_REAR_WALL - .12),
                (-half_x, WING_REAR_WALL - .12),
                (-half_x, -half_z),
            ),
            profile,
            overlap=overlap,
...
def perimeter_wall(wall: str, floor: float, top: float, base_lift: float = 0.0) -> kit.Geometry:
    """Author an L-shaped closed perimeter; every window cuts its own wall plane."""
    if wall == "north":
        segments = ((UPPER_PARTITION_X, 5.0, 4.0), (-5.0, UPPER_PARTITION_X, WING_REAR_WALL))
    elif wall == "west":
        segments = ((-4.0, WING_REAR_WALL, -5.0),)
    elif wall == "east":
        segments = ((-4.0, 4.0, 5.0), (4.0, WING_REAR_WALL, UPPER_PARTITION_X))
    else:
        segments = ((-5.0, 5.0, -4.0),)
    runs = []
    for low, high, plane in segments:
        middle = (low + high) * 0.5
        openings = [kit.Opening(row["across"] - middle, row["width"],
                               row["head"] - base_lift, row["sill"] - base_lift)
                    for row in WINDOWS if row["wall"] == wall and
                    abs(row["wall_plane"] - plane) < .001 and
                    abs(row["floor_elevation"] - floor) < .001 and
                    row["sill"] < top - floor]
        if wall == "south" and floor == 0.0:
            openings.append(kit.Opening(DOOR_CENTER_X - middle, DOOR_WIDTH, DOOR_HEIGHT, 0.0))
        geometry = kit.wall_run(high - low, top - floor - base_lift,
                                UPPER_PARTITION_THICKNESS if wall == "east" and plane == UPPER_PARTITION_X else WALL_THICKNESS,
                                openings, chamfer=0.0,
                                base=floor + base_lift)
        if wall in ("east", "west"):
            geometry = kit.translated(kit.rotated_z(geometry, 90.0), (plane, middle, 0.0))
        else:
            geometry = kit.translated(geometry, (middle, plane, 0.0))
...
def build_shell(asset: AssetBuild, materials: dict) -> None:
    ...
    for wall, base_name in (("east", "EastCutaway"), ("west", "Left")):
        ...
        base = perimeter_wall(wall, 0.0, 0.62)
    ...
    floor = merge((kit.floor_slab(ROOM_WIDTH, ROOM_DEPTH, FLOOR_THICKNESS, 0.012),
                   kit.chamfered_box(((-5.0 + UPPER_PARTITION_X) * .5, 4.45, -FLOOR_THICKNESS * .5),
                                      (UPPER_PARTITION_X + 5.0, .90, FLOOR_THICKNESS), .006)))
    add_part(
        asset,
        materials,
        "FIX_Floor",
    ...
    add_part(
        ...
        materials,
        "FIX_Cornice",
        kit.sweep(((-4.88, -3.88), (4.88, -3.88), (4.88, 3.88),
                   (-1.83, 3.88), (-1.83, 4.78), (-4.88, 4.78), (-4.88, -3.88)),
                  [(x, y + ROOM_HEIGHT - .105) for x, y in kit.profile_cornice(.105, .075)]),
        "cornice",
        "DarkWood",
        unity_space=False,
    )
...
def build_upper_storey(asset: AssetBuild, materials: dict) -> None:
    ...
    slab_regions = (
        ...
        (-5.0, UPPER_PARTITION_X, 4.0, WING_REAR_WALL),
    )
    add_part(
        asset,
        materials,
    ...
    for wall in ("east", "west"):
        direction = 1 if wall == "east" else -1
        base = perimeter_wall(wall, UPPER_FLOOR_ELEVATION, UPPER_FLOOR_ELEVATION + 0.62)
    ...
    add_part(
        ...
        merge((bp.u_box(
            ...
            (ROOM_WIDTH, CEILING_THICKNESS, ROOM_DEPTH), .006),
            u_box_limits(-5.0, UPPER_PARTITION_X, UPPER_CEILING_HEIGHT,
                         UPPER_CEILING_HEIGHT + CEILING_THICKNESS, 4.0, WING_REAR_WALL))),
        "upper_ceiling",
        "CeilingPlaster",
    )
...
def build_windows(asset: AssetBuild, materials: dict) -> None:
    for index, x in enumerate(WINDOW_CENTERS_X):
        ...
        plane = next(row["wall_plane"] for row in WINDOWS if row["stable_id"] == "ground-north-" + side.lower())
        frame = kit.door_frame(
            WINDOW_WIDTH,
        ...
        add_part(
            ...
            bp.u_plate(
                (x, (WINDOW_SILL + WINDOW_HEAD) * 0.5, plane - .18),
        ...
        for direction in (-1.0, 1.0):
            ...
            for fold_index in range(2):
                ...
                folds.append(bp.u_box(
                    (edge_x + offset, 1.55, plane - .28 - fold_index * 0.015),
...
def build_additional_windows(asset: AssetBuild, materials: dict) -> None:
    for row in WINDOWS:
        ...
        if wall in ("north", "south"):
            origin = (row["across"], row["wall_plane"] - (1 if wall == "north" else -1) * .06,
                      row["floor_elevation"] + row["sill"])
        else:
            origin = (row["wall_plane"] - (1 if wall == "east" else -1) * .06, row["across"],
                      row["floor_elevation"] + row["sill"])
        ...
        add_part(asset, materials, row["glass_part"], placed(pane),
                 ...
                 tint=(0.48, 0.57, 0.58, 0.92) if row.get("frosted") else (0.16, 0.24, 0.28, 0.65))
...
def build_upper_corridor_extras(asset: AssetBuild, materials: dict) -> None:
    """Wall storage and one practical; the cleaning set belongs in the bathroom."""
    ...
    # The broom leans south into the bathroom wall, clear of the doorway and
    # the toilet. Its old outward lean occupied the stair landing's turn.
    pail.append(bp.u_rotated_about(
        bp.u_box(
            (x_max - 0.14, floor + 0.62, z_min + 0.17),
            (0.032, 1.24, 0.032),
            0.006,
        ),
        (-10.0, 0.0, 5.0),
        (x_max - 0.14, floor, z_min + 0.17),
    ))
    pail.append(bp.u_box(
        (x_max - 0.20, floor + 0.055, z_min + 0.18),
        (0.085, 0.105, 0.30),
        0.010,
    ))
    add_part(
    ...
    # A simple old ceiling fitting: pressed mounting plate, two fasteners,
    # short socket and a real thick glass envelope around its warm bulb.
    lamp_x, lamp_y, lamp_z = ANCHORS_UNITY["ANCHOR_UpperCorridorLampLight"]
    mounting = [
        bp.u_cylinder((lamp_x, 5.8825, lamp_z), (.175, .0175, .175), 16),
        bp.u_cylinder((lamp_x, 5.798, lamp_z), (.065, .075, .065), 12),
        bp.u_cylinder((lamp_x, 5.718, lamp_z), (.085, .014, .085), 16),
    ]
    for offset in (-.061, .061):
        mounting.append(bp.u_cylinder((lamp_x + offset, 5.860, lamp_z),
                                       (.014, .006, .014), 8))
    add_part(asset, materials, "DRESS_UpperCorridor.CeilingLamp", merge(mounting),
             "upper_corridor_lamp", "PaintedMetal", tint=(.56, .52, .41, 1.0))
    glass_profile = (
        (.040, 5.720), (.105, 5.708), (.154, 5.664), (.158, 5.620),
        (.130, 5.560), (.065, 5.522), (.020, 5.520),
        (.020, 5.528), (.062, 5.530), (.122, 5.567), (.149, 5.620),
        (.145, 5.660), (.100, 5.700), (.040, 5.712),
    )
    glass = kit.translated(kit.lathe(glass_profile, 20), (lamp_x, lamp_z, 0.0))
    if bp.signed_volume(glass) < 0.0:
        glass = (glass[0], [tuple(reversed(face)) for face in glass[1]])
    add_part(asset, materials, "DRESS_UpperCorridor.LampGlass", glass,
             "upper_corridor_lamp_glass", "Glass", unity_space=False,
             emissive=True, casts_shadows=False, tint=(.94, .78, .52, .62))
    bulb = kit.translated(kit.lathe(((.008, lamp_y - .046),
                                    (.032, lamp_y - .033), (.037, lamp_y),
                                    (.024, lamp_y + .030), (.018, lamp_y + .065)), 12),
                         (lamp_x, lamp_z, 0.0))
    add_part(asset, materials, "DRESS_UpperCorridor.LampBulb", bulb,
             "upper_corridor_lamp_bulb", "Ceramic", unity_space=False,
             emissive=True, casts_shadows=False, tint=(1.0, .82, .52, 1.0))
...
def build_upper_skirting(asset: AssetBuild, materials: dict) -> None:
    ...
    for room_z_far, divider_face, door_center in (
        ...
        run_z(divider_face, door_center - outward * door_half, room_face, -1.0)
        run_z(door_center + outward * door_half, room_z_far, room_face, -1.0)

    corridor_far_z = BATHROOM_PARTITION_Z - .08
    run_z(-ROOM_DEPTH * .5, UPPER_DOOR_CENTERS_Z[0] - door_half,
          corridor_face, 1.0)
...
def add_flame_layer(asset: AssetBuild, materials: dict, name: str,
                    tongues: Sequence[tuple], front: bool) -> None:
    """Closed, curved flame volumes; UV1 is a tongue-local thermal field.

    Eight irregular sections keep a broad rooted body, a neck and a curling
    tip. Each tongue has its own phase; runtime may bend the tips but must
    never scale/rotate the merged mesh around the house's authoring origin.
    """
    levels = (0.0, .10, .23, .39, .57, .73, .88, 1.0)
    widths = (.55, .94, 1.0, .84, .61, .39, .17, .014)
    vertices, faces, uv, colors = [], [], [], []
    sides = 8
    for x, y, z, height, width, depth, lean, curl, phase, heat in tongues:
        rings = []
        for v, spread in zip(levels, widths):
            centre_x = x + lean * v ** 1.25 + curl * math.sin(v * math.pi) * v
            centre_z = z + .025 * math.sin(v * math.pi * 1.4 + phase * math.tau) * v
            ring = []
            for side in range(sides):
                angle = math.tau * side / sides
                # Small unequal shoulders avoid seven copies of one profile.
                shoulder = 1.0 + .10 * math.sin(angle * 3 + phase * math.tau + v * 4)
                ring.append(len(vertices))
                vertices.append((centre_x + math.cos(angle) * width * .5 * spread * shoulder,
                                 y + v * height,
                                 centre_z + math.sin(angle) * depth * .5 * spread))
                uv.append((.5 + .5 * math.cos(angle), v))
                colors.append((phase, heat, 1.0 if front else 0.0, 1.0))
            rings.append(ring)
    ...
    obj = add_part(asset, materials, name, (vertices, faces), "fire_flame", "Fire",
                   emissive=True, casts_shadows=False,
                   tint=(1.0, .70, .26, 1.0) if front else (1.0, .47, .12, 1.0))
    field = obj.data.uv_layers.new(name="FlameField")
    thermal = obj.data.color_attributes.new(name="FlameThermal", type="FLOAT_COLOR", domain="POINT")
    for loop in obj.data.loops:
        field.data[loop.index].uv = uv[loop.vertex_index]
    for index, color in enumerate(colors):
        thermal.data[index].color = color
    for polygon in obj.data.polygons:
        polygon.use_smooth = True
    asset.parts[-1].flame_uv = uv
    asset.parts[-1].flame_colors = colors
    obj["bp_flame_tongues"] = len(tongues)
    obj["bp_flame_sections"] = len(levels)


def fire_log(center: Sequence[float], length: float, radius: float,
             yaw: float, phase: float) -> kit.Geometry:
    """Three uneven bark rings and blunt split ends, rather than smooth rods."""
    vertices, faces, rings = [], [], []
    for row, x in enumerate((-length * .5, -.035, length * .5)):
        ring = []
        for side in range(8):
            angle = side * math.tau / 8
            r = radius * (1.0 + .16 * math.sin(side * 2.7 + phase + row * .8))
            ring.append(len(vertices))
    ...
    for low, high in zip(rings, rings[1:]):
        for side in range(8):
            following = (side + 1) % 8
            faces.append((low[side], low[following], high[following], high[side]))
    geometry = (vertices, faces)
    if bp.signed_volume(geometry) < 0:
        geometry = (vertices, [tuple(reversed(face)) for face in faces])
    return kit.translated(bp.u_rotated(geometry, (0, yaw, 0)), center)
...
def build_fireplace(asset: AssetBuild, materials: dict) -> None:
    ...
    add_part(
        ...
        # Rear lining is behind the fuel. The old front panel intersected
        # the fire light and sealed the entire visible opening with a wall.
        merge((bp.u_box((0.0, 0.73, 3.875), (1.30, 1.05, 0.08), 0.016),
               bp.u_box((-.645, .74, 3.60), (.035, 1.06, .50), .006),
               bp.u_box((.645, .74, 3.60), (.035, 1.06, .50), .006))),
        "firebox",
        "DarkWood",
        tint=(0.15, 0.13, 0.115, 1.0),
    )

    logs = merge((fire_log((-.04, .285, 3.49), 1.03, .078, -5, .4),
                  fire_log((.02, .292, 3.71), .93, .073, 4, 1.8),
                  fire_log((-.01, .365, 3.61), .72, .060, 20, 3.1)))
    add_part(
        asset,
        materials,
        "FIX_Fire.Logs",
        ...
        "fire_logs",
        "DarkWood",
        tint=(0.25, 0.21, 0.18, 1.0),
    )
    ash = []
    embers = []
    for index in range(15):
        x = -.48 + (index % 5) * .235 + .019 * math.sin(index * 2.3)
        z = 3.39 + (index // 5) * .145 + .014 * math.cos(index * 1.7)
        ash.append(bp.u_tapered_cylinder((x, .22, z),
                   (.10 + .022 * (index % 3), .018, .075), .64, 5))
        if index % 3 != 1:
            embers.append(bp.u_tapered_cylinder((x + .024, .239, z - .015),
                          (.035 + .008 * (index % 2), .008, .033), .45, 5))
    # Glowing fissures interrupt the near log, instead of luminous pancakes
    # floating in front of an unburnt cylinder.
    for index in range(7):
        x = -.43 + index * .137 + .019 * math.sin(index * 2.2)
        y = .293 + .012 * math.sin(index * 1.6)
        embers.append(kit.translated(bp.u_rotated(
            bp.u_box((0, 0, 0), (.038 + .019 * (index % 3), .007, .006)),
            (0, 0, -13 + (index % 4) * 8)), (x, y, 3.414)))
    add_part(asset, materials, "FIX_Fire.Ash", merge(ash),
             "fire_ash", "Concrete", tint=(.21, .19, .16, 1.0))
    add_part(
        asset,
        materials,
        "FIX_Fire.Embers",
        ...
        emissive=True,
        casts_shadows=False,
        tint=(1.0, 0.21, 0.025, 1.0),
    )
    add_flame_layer(asset, materials, "FIX_Fire.Flame.Back", (
        (-.38, .28, 3.66, .59, .20, .105, -.07, .09, .08, .66),
        (-.09, .27, 3.70, .84, .27, .110, .09, -.12, .35, .78),
        (.27, .30, 3.66, .70, .245, .095, -.12, .08, .61, .73)), False)
    add_flame_layer(asset, materials, "FIX_Fire.Flame.Front", (
        (-.36, .285, 3.46, .37, .18, .085, .07, -.08, .18, .94),
        (-.17, .30, 3.46, .58, .24, .080, -.09, .12, .44, 1.0),
        (.11, .28, 3.44, .49, .26, .085, .09, -.11, .70, .95),
        (.38, .285, 3.50, .42, .16, .080, -.05, .085, .90, .88)), True)
...
def build_floor_lamp(asset: AssetBuild, materials: dict) -> None:
    ...
    # A 5 mm fabric shell is open above and below; the shade is neither a
    # solid cone nor a shadow-casting plug around the practical's source.
    shade_vertices, shade_faces, shade_rings = [], [], []
    for radius, y in ((.31, 1.27), (.1705, 1.81), (.1655, 1.81), (.305, 1.27)):
        row = []
        for side in range(16):
            angle = math.tau * side / 16
            row.append(len(shade_vertices))
            shade_vertices.append((floor_x + radius * math.cos(angle), y,
                                   floor_z + radius * math.sin(angle)))
        shade_rings.append(row)
    for row in range(4):
        low, high = shade_rings[row], shade_rings[(row + 1) % 4]
        for side in range(16):
            following = (side + 1) % 16
            shade_faces.append((low[following], low[side], high[side], high[following]))
    add_part(
        asset,
        materials,
        "DRESS_FloorLamp.Shade",
        (shade_vertices, shade_faces),
        "floor_lamp_shade",
        "BedLinen",
        casts_shadows=False,
...
def bathroom_vessel(center_x: float, center_z: float,
                    sections: Sequence[tuple[float, float, float]],
                    segments: int = 24, square: float = .62) -> kit.Geometry:
    """Closed porcelain solid following outside, lip, inside and bowl floor.

    The profile folds back down inside the vessel: its mouth is a real cavity,
    never a dark oval pasted over a box. Coordinates and winding are Unity's.
    """
    vertices = []
    for height, width, depth in sections:
        for index in range(segments):
            angle = math.tau * index / segments
            c, sn = math.cos(angle), math.sin(angle)
            vertices.append((center_x + math.copysign(abs(c) ** square, c) * width * .5,
                             UPPER_FLOOR_ELEVATION + height,
                             center_z + math.copysign(abs(sn) ** square, sn) * depth * .5))
    faces = []
    ...
    faces.append(tuple(range((len(sections) - 1) * segments, len(vertices))))
    geometry = (vertices, faces)
    if bp.signed_volume(geometry) < 0:
        geometry = (vertices, [tuple(reversed(face)) for face in faces])
    return geometry
...
def build_bathroom(asset: AssetBuild, materials: dict) -> None:
    """A complete, modest shared bathroom, passive like the other upper rooms."""
    floor = UPPER_FLOOR_ELEVATION
    door_half = BATHROOM_DOOR_WIDTH * .5
    wall_geometry = []
    for x0, x1 in ((-5.0, BATHROOM_DOOR_X - door_half),
                   (BATHROOM_DOOR_X + door_half, UPPER_PARTITION_X)):
        wall_geometry.append(u_box_limits(x0, x1, floor, UPPER_CEILING_HEIGHT,
                                          1.82, 1.98, .003))
    wall_geometry.append(u_box_limits(BATHROOM_DOOR_X - door_half,
                                      BATHROOM_DOOR_X + door_half,
                                      floor + UPPER_DOOR_HEIGHT, UPPER_CEILING_HEIGHT,
                                      1.82, 1.98, .003))
    add_part(asset, materials, "FIX_BathroomSouthWall", merge(wall_geometry),
             "bathroom_partition", "Wallpaper")
    frame = kit.translated(kit.door_frame(BATHROOM_DOOR_WIDTH, UPPER_DOOR_HEIGHT,
                                          .20, jamb=.065, architrave=.012),
                           (BATHROOM_DOOR_X, BATHROOM_PARTITION_Z, floor))
    add_part(asset, materials, "FIX_BathroomDoor.Frame", frame,
             "bathroom_door_frame", "DarkWood", unity_space=False)
    leaf = merge((
        bp.u_box((-1.90, floor + 1.075, 2.59), (.065, 2.15, 1.13), .014),
        bp.u_box((-1.945, floor + .57, 2.59), (.025, .70, .86), .009),
        bp.u_box((-1.945, floor + 1.55, 2.59), (.025, .70, .86), .009)))
    add_part(asset, materials, "FIX_BathroomDoor.OpenLeaf", leaf,
             "bathroom_door", "PaleWood")

    # A flush thin stone finish over the structural slab; a sober joint grid
    # gives the wet room its own material scale without retexturing the atlas.
    tiles = []
    for ix in range(7):
        x0 = BATHROOM_X[0] + ix * (BATHROOM_X[1] - BATHROOM_X[0]) / 7
        x1 = BATHROOM_X[0] + (ix + 1) * (BATHROOM_X[1] - BATHROOM_X[0]) / 7
        for iz in range(6):
            z0 = BATHROOM_Z[0] + iz * (BATHROOM_Z[1] - BATHROOM_Z[0]) / 6
            z1 = BATHROOM_Z[0] + (iz + 1) * (BATHROOM_Z[1] - BATHROOM_Z[0]) / 6
            tiles.append(u_box_limits(x0 + .003, x1 - .003, floor + .020,
                                      floor + .026, z0 + .003, z1 - .003, 0.0))
    add_part(asset, materials, "FIX_BathroomFloor", merge(tiles),
             "bathroom_floor", "Concrete", tint=(.76, .73, .63, 1.0))
    # Splashback only where water belongs; upper lime plaster stays visible.
    splash = []
    for ix in range(10):
        for iy in range(3):
            splash.append(bp.u_box((-4.84 + (ix + .5) * .296,
                                    floor + .14 + iy * .265, 4.762),
                                   (.290, .259, .021), 0.0))
    for iz in range(3):
        for iy in range(2):
            splash.append(bp.u_box((-1.852, floor + .985 + iy * .25,
                                    3.15 + (iz + .5) * .22),
                                   (.020, .244, .214), 0.0))
    add_part(asset, materials, "FIX_BathroomSplashback", merge(splash),
             "bathroom_splashback", "Ceramic", tint=(.83, .86, .76, 1.0))

    tub = bathroom_vessel(-3.70, 4.305,
        ((.035, 1.50, .59), (.10, 1.62, .68), (.535, 1.68, .735),
         (.580, 1.70, .75), (.580, 1.54, .585), (.50, 1.49, .54),
         (.19, 1.30, .40), (.16, 1.22, .34)))
    add_part(asset, materials, "FIX_BathroomTub", tub,
             "bathroom_tub", "Ceramic", tint=(.90, .87, .77, 1.0))
    # Ordinary faucet, drain and two cross handles. No flow or standing water.
    plumbing = [bp.u_cylinder((-3.03, floor + .168, 4.305), (.07, .003, .07), 12)]
    for x in (-3.43, -3.97):
        plumbing.extend((bp.u_cylinder((x, floor + .62, 4.65), (.07, .065, .07), 10),
                         bp.u_box((x, floor + .69, 4.65), (.13, .024, .030), .005),
                         bp.u_box((x, floor + .69, 4.65), (.030, .024, .13), .005)))
    plumbing.extend((bp.u_cylinder((-3.70, floor + .69, 4.65), (.065, .11, .065), 10),
                     bp.u_box((-3.70, floor + .795, 4.57), (.065, .06, .20), .014),
                     bp.u_cylinder((-3.70, floor + .758, 4.475), (.065, .05, .065), 10)))
    add_part(asset, materials, "DRESS_Bathroom.TubFittings", merge(plumbing),
             "bathroom_fittings", "PaintedMetal", tint=(.56, .55, .46, 1.0))

    # Low tank at the west wall; bowl faces east and has a real open seat.
    toilet = merge((bathroom_vessel(-4.375, 2.565,
                    ((.025, .26, .25), (.11, .30, .27), (.28, .38, .34),
                     (.40, .57, .45), (.425, .57, .45), (.425, .42, .30),
                     (.31, .33, .24), (.20, .16, .15)), 20, .87),
                    bp.u_box((-4.695, floor + .545, 2.565), (.17, .55, .40), .025),
                    bp.u_box((-4.695, floor + .818, 2.565), (.18, .024, .42), .008)))
    add_part(asset, materials, "FIX_BathroomToilet", toilet,
             "bathroom_toilet", "Ceramic", tint=(.88, .85, .76, 1.0))
    seat = bathroom_vessel(-4.375, 2.565,
                          ((.43, .585, .45), (.462, .585, .45),
                           (.462, .43, .30), (.43, .43, .30)), 20, .87)
    add_part(asset, materials, "DRESS_Bathroom.ToiletSeat", seat,
             "bathroom_toilet_seat", "PaleWood", tint=(.71, .64, .46, 1.0))

    vanity = [bp.u_box((-2.145, floor + .405, 3.49), (.51, .69, .66), .015)]
    for z in (3.285, 3.695):
        vanity.extend((bp.u_box((-2.411, floor + .43, z), (.018, .48, .29), .008),
                       bp.u_box((-2.428, floor + .48, z), (.024, .024, .08), .008)))
    for x in (-2.35, -1.94):
        for z in (3.22, 3.76):
            vanity.append(bp.u_box((x, floor + .07, z), (.065, .14, .065), .008))
    add_part(asset, materials, "FIX_BathroomVanity", merge(vanity),
             "bathroom_vanity", "PaleWood", tint=(.69, .53, .32, 1.0))
    basin = bathroom_vessel(-2.145, 3.49,
                            ((.755, .45, .56), (.85, .55, .70), (.86, .55, .70),
                             (.86, .40, .52), (.79, .25, .36)), 20, .66)
    add_part(asset, materials, "DRESS_Bathroom.Basin", basin,
             "bathroom_basin", "Ceramic", tint=(.88, .85, .76, 1.0))
    tap = merge((bp.u_cylinder((-1.94, floor + .94, 3.49), (.04, .10, .04), 10),
                 bp.u_box((-2.015, floor + 1.025, 3.49), (.18, .04, .04), .009),
                 bp.u_cylinder((-2.10, floor + 1.00, 3.49), (.04, .03, .04), 10)))
    add_part(asset, materials, "DRESS_Bathroom.BasinTap", tap,
             "bathroom_fittings", "PaintedMetal")

    mirror = merge((bp.u_box((-1.89, floor + 1.43, 3.49), (.045, .72, .60), .018),
                    bp.u_box((-1.94, floor + 1.045, 3.49), (.15, .045, .70), .010)))
    add_part(asset, materials, "FIX_BathroomMirror", mirror,
             "bathroom_mirror_frame", "PaleWood")
    add_part(asset, materials, "DRESS_Bathroom.MirrorGlass",
             bp.u_box((-1.917, floor + 1.43, 3.49), (.013, .62, .50), .005),
             "bathroom_mirror_glass", "Glass", casts_shadows=False,
             tint=(.60, .65, .61, 1.0))

    # The visible opal fitting explains its sole local warm light.
    lamp = merge((bp.u_box((-1.88, 5.25, 3.55), (.055, .20, .20), .024),
                  bp.u_box((-2.015, 5.25, 3.55), (.23, .06, .06), .015)))
    add_part(asset, materials, "FIX_BathroomLamp", lamp,
             "bathroom_lamp", "PaintedMetal")
    add_part(asset, materials, "DRESS_Bathroom.LampGlobe",
             bathroom_vessel(-2.13, 3.55, ((1.60, .09, .09), (1.65, .19, .19),
                              (1.76, .19, .19), (1.81, .09, .09)), 16, 1.0),
             "bathroom_lamp_globe", "Ceramic", emissive=True, casts_shadows=False)

    # A few quiet useful things; floor furniture stays in the west pocket.
    basket = bathroom_vessel(-4.52, 3.43, ((.04, .39, .44), (.46, .47, .54),
                              (.49, .47, .54), (.49, .41, .48), (.085, .34, .39)), 16, .72)
    add_part(asset, materials, "DRESS_Bathroom.LaundryBasket", basket,
             "bathroom_laundry_basket", "Wicker", tint=(.63, .50, .30, 1.0))
    rail = merge((bp.u_box((-4.83, floor + 1.13, 3.43), (.07, .04, .58), .009),
                  bp.u_box((-4.855, floor + 1.13, 3.17), (.09, .09, .055), .009),
                  bp.u_box((-4.855, floor + 1.13, 3.69), (.09, .09, .055), .009)))
    add_part(asset, materials, "DRESS_Bathroom.TowelRail", rail,
             "bathroom_fittings", "PaintedMetal")
    textiles = [bp.u_box((-4.77 - i * .008, floor + .89 + i * .015, 3.33 + i * .10),
                         (.033, .48 - i * .03, .115), .012) for i in range(3)]
    textiles.append(bp.u_box((-4.52, floor + .45, 3.43), (.33, .07, .39), .025))
    add_part(asset, materials, "FIX_BathroomTextiles", merge(textiles),
             "bathroom_textiles", "TeaCloth", tint=(.82, .80, .69, 1.0))
    add_part(asset, materials, "DRESS_Bathroom.Soap",
             bp.u_box((-1.985, floor + .882, 3.74), (.09, .035, .06), .011),
             "bathroom_soap", "Ceramic", tint=(.72, .77, .64, 1.0))
...
def build() -> AssetBuild:
    ...
    build_bathroom(asset, materials)
...
def validate_window_layout(asset: AssetBuild, problems: list[str]) -> None:
    ...
    if len(WINDOWS) != 17 or len({row["stable_id"] for row in WINDOWS}) != 17:
        problems.append("the shared window table requires seventeen distinct openings")
    for wall in ("north", "south", "east", "west"):
        for floor in (0.0, UPPER_FLOOR_ELEVATION):
            if sum(row["wall"] == wall and row["floor_elevation"] == floor
                   for row in WINDOWS) != (3 if wall == "north" and floor > 0 else 2):
                problems.append(f"{wall} floor {floor} lost its matching window rhythm")
...
def validate_bathroom(asset: AssetBuild, problems: list[str]) -> None:
    from mathutils.bvhtree import BVHTree

    by_name = {part.name: part for part in asset.parts}
    required = {"FIX_BathroomTub", "FIX_BathroomToilet", "FIX_BathroomVanity",
                "FIX_BathroomMirror", "FIX_BathroomLamp", "FIX_BathroomTextiles",
                "FIX_BathroomSouthWall", "FIX_BathroomDoor.Frame",
                "FIX_BathroomFloor", "DRESS_Bathroom.LaundryBasket"}
    for missing in sorted(required - by_name.keys()):
        problems.append(f"bathroom semantic part '{missing}' is absent")
    if required - by_name.keys():
        return
    # Every main fixture stays at human scale in its own real room. A clear
    # central lane reaches the tub between the west fixtures and east basin.
    blocking = ("FIX_BathroomTub", "FIX_BathroomToilet", "FIX_BathroomVanity",
                "DRESS_Bathroom.LaundryBasket", "DRESS_UpperCorridor.Pail")
    for name in blocking:
        low, high = bounds_for(by_name[name])
        if (low[0] < BATHROOM_X[0] - .002 or high[0] > BATHROOM_X[1] + .002 or
                low[2] < BATHROOM_Z[0] - .002 or high[2] > BATHROOM_Z[1] + .002):
            problems.append(f"{name} crosses the bathroom wall")
        for x, z in ((-2.505, 2.40), (-3.20, 2.80), (-3.20, 3.45)):
            near_x, near_z = min(max(x, low[0]), high[0]), min(max(z, low[2]), high[2])
            if math.hypot(near_x - x, near_z - z) < .32:
                problems.append(f"{name} blocks the bathroom capsule route")
    # A downward ray must enter the tub and hit its bottom, not a false lid.
    tub = by_name["FIX_BathroomTub"]
    hit = BVHTree.FromPolygons(*tub.geometry).ray_cast(
        Vector(source_point((-3.70, UPPER_FLOOR_ELEVATION + .90, 4.305))),
        Vector(source_point((0.0, -1.0, 0.0))), 1.0)
    if hit[0] is None or abs(unity_point(hit[0])[1] - UPPER_FLOOR_ELEVATION - .16) > .003:
        problems.append("the bath has no real open cavity and floor")
    west_window = next(row for row in WINDOWS if row["stable_id"] == "upper-west-north")
    if west_window["across"] - west_window["width"] * .5 <= BATHROOM_Z[0] + .08:
        problems.append("the west bathroom window intersects its south partition")
    if any(z >= UPPER_DOOR_CENTERS_Z[1] - UPPER_DOOR_WIDTH * .5
           for z in (CORRIDOR_PEG_Z[1], CORRIDOR_SHELF_Z[1])):
        problems.append("corridor dressing crosses the relocated bedroom doorway")
    skirting_tree = BVHTree.FromPolygons(*by_name["FIX_UpperSkirting"].geometry)
    for door_z in UPPER_DOOR_CENTERS_Z:
        hit = skirting_tree.ray_cast(
            Vector(source_point((-2.0, UPPER_FLOOR_ELEVATION + .08, door_z))),
            Vector(source_point((1.0, 0.0, 0.0))), .6)
        if hit[0] is not None:
            problems.append("upper skirting bridges a bedroom's clear doorway")

    # The former cleaning-set position obstructed the right turn immediately
    # after the last stair. Check the whole actual mesh, including the broom.
    pail_low, pail_high = bounds_for(by_name["DRESS_UpperCorridor.Pail"])
    for index in range(17):
        x = -4.0 + (-2.45 + 4.0) * index / 16
        z = -3.30
        near_x = min(max(x, pail_low[0]), pail_high[0])
        near_z = min(max(z, pail_low[2]), pail_high[2])
        if math.hypot(near_x - x, near_z - z) < .32:
            problems.append("the cleaning set obstructs the upper landing turn")

    lamp_names = ("DRESS_UpperCorridor.CeilingLamp", "DRESS_UpperCorridor.LampGlass",
                  "DRESS_UpperCorridor.LampBulb")
    for name in lamp_names:
        if name not in by_name:
            problems.append(f"corridor practical has no '{name}'")
    if all(name in by_name for name in lamp_names):
        lamp = ANCHORS_UNITY["ANCHOR_UpperCorridorLampLight"]
        for name in lamp_names[1:]:
            low, high = bounds_for(by_name[name])
            if any(lamp[i] < low[i] or lamp[i] > high[i] for i in range(3)):
                problems.append(f"corridor light is outside its visible {name}")
            if low[1] < UPPER_FLOOR_ELEVATION + 1.95:
                problems.append("the corridor fitting lost standing clearance")
        mount_low, mount_high = bounds_for(by_name[lamp_names[0]])
        if abs(mount_high[1] - UPPER_CEILING_HEIGHT) > .001:
            problems.append("corridor lamp mounting no longer meets its ceiling")


def validate_hearth_and_practicals(asset: AssetBuild, problems: list[str]) -> None:
    from mathutils.bvhtree import BVHTree

    by_name = {part.name: part for part in asset.parts}
    for name, tongue_count in (("FIX_Fire.Flame.Back", 3), ("FIX_Fire.Flame.Front", 4)):
        part = by_name[name]
        if (len(part.geometry[0]) != tongue_count * 8 * 8 or
                len(part.flame_uv) != len(part.geometry[0]) or
                len(part.flame_colors) != len(part.geometry[0]) or
                len(part.obj.data.uv_layers) != 2):
            problems.append(f"{name} lost its eight-section tongue-local fields")
        if part.casts_shadows or not part.emissive:
            problems.append(f"{name} must transmit its real hearth light")
        for index, (u, v) in enumerate(part.flame_uv):
            if not (0 <= u <= 1 and 0 <= v <= 1):
                problems.append(f"{name} has invalid thermal UV1")
            x, y, z = unity_point(part.geometry[0][index])
            # Runtime sway is at most 4 cm, fades to zero at the rooted base.
            padding = .04 * v * v
            if (abs(x) + padding >= .625 or y - padding < .20 or y + padding > 1.29 or
                    z - padding < 3.34 or z + padding > 3.83):
                problems.append(f"{name} animated envelope leaves the open firebox")
                break
        for tongue in range(tongue_count):
            section = part.flame_uv[tongue * 64:(tongue + 1) * 64]
            if len({v for _, v in section}) != 8 or any(v != 0 for _, v in section[:8]):
                problems.append(f"{name} has no rooted eight-section tongue {tongue}")
    opaque = merge(part.geometry for part in asset.parts
                   if part.role in {"firebox", "fireplace_stone", "fire_logs", "fire_ash"})
    tree = BVHTree.FromPolygons(*opaque)
    light = ANCHORS_UNITY["ANCHOR_FireLight"]
    # The point and its entire motion envelope must illuminate the opening;
    # checking a render flag alone would miss a light sealed inside a mesh.
    for dx in (-.04, 0, .04):
        for dy in (-.04, 0, .04):
            for dz in (-.04, .04):
                start = Vector(source_point((light[0] + dx, light[1] + dy, light[2] + dz)))
                if tree.ray_cast(start, Vector(source_point((0, 0, -1))), .5)[0] is not None:
                    problems.append("an opaque surface seals the hearth light from the room")
    transmissive = ("DRESS_FloorLamp.Shade", "DRESS_FloorLamp.Bulb",
                    "DRESS_UpperNorth.LampBulb", "DRESS_UpperSouth.LampBulb",
                    "DRESS_Bathroom.LampGlobe", "DRESS_UpperCorridor.LampGlass",
                    "DRESS_UpperCorridor.LampBulb")
    if any(by_name[name].casts_shadows for name in transmissive):
        problems.append("a cloth/opal practical traps its own source with an opaque shadow")
    shade = by_name["DRESS_FloorLamp.Shade"]
    lamp = ANCHORS_UNITY["ANCHOR_FloorLampLight"]
    if BVHTree.FromPolygons(*shade.geometry).ray_cast(Vector(source_point(lamp)),
            Vector(source_point((0, 1, 0))), .5)[0] is not None:
        problems.append("the floor-lamp shade has a solid lid")
    if lamp != (-1.95, 1.50, -1.62):
        problems.append("the floor lamp left the sofa's near southeast corner")
    shade_low, shade_high = bounds_for(shade)
    sofa_low, _ = bounds_for(by_name["FIX_Sofa.Frame"])
    if sofa_low[2] - shade_high[2] < .10:
        problems.append("the floor-lamp shade intersects the sofa's near arm")
    for index in range(65):
        t = index / 64
        x, z = -1.51 * t, -3.30 + 2.70 * t
        near_x = min(max(x, shade_low[0]), shade_high[0])
        near_z = min(max(z, shade_low[2]), shade_high[2])
        if math.hypot(x - near_x, z - near_z) < .32:
            problems.append("the floor lamp blocks the entry-to-sofa capsule route")
            break


def validate(asset: AssetBuild) -> dict:
    problems: list[str] = []
    validate_hearth_and_practicals(asset, problems)
    names: set[str] = set()
    forbidden = (
    ...
    expected_names = {
        ...
        "DRESS_UpperCorridor.CeilingLamp", "DRESS_UpperCorridor.LampGlass",
        "DRESS_UpperCorridor.LampBulb",
    }
    ...
    validate_bathroom(asset, problems)
    ...
    required_roles = {
        ...
        "upper_corridor_lamp", "upper_corridor_lamp_glass", "upper_corridor_lamp_bulb",
    }
    ...
    if set(asset.anchors) != set(ANCHORS_UNITY):
        problems.append("the semantic anchor contract drifted")
    ...
    if abs(west_window[2] - east_window[2] - .90) > 0.001:
        problems.append("the west window no longer follows the extended niche")
    ...
    if len(UPPER_DOOR_CENTERS_Z) != 2 or UPPER_DOOR_WIDTH < 1.20:
        problems.append("the bedroom partition requires exactly two clear doors")
    ...
    if len(asset.parts) > 160:
        problems.append(f"{len(asset.parts)} meshes exceed the 160-mesh cap")
    if triangles > 22000:
        problems.append(f"{triangles} triangles exceed the 22000 cap")
        ...
        "parts": [
            {
                ...
                "flame_uv1": [[stable(value) for value in uv] for uv in part.flame_uv],
                "flame_colors": [[stable(value) for value in color] for color in part.flame_colors],
            }
            for part in asset.parts
        ],
        "anchors": {
...
def manifest_part(part: Part) -> dict:
    ...
    return {
        ...
        "uv_channel_count": len(part.obj.data.uv_layers),
        "flame_field_vertex_count": len(part.flame_uv),
        "bounds_min": [stable(value) for value in source_low],
        "bounds_max": [stable(value) for value in source_high],
        "unity_bounds_min": [stable(value) for value in unity_low],
        "unity_bounds_max": [stable(value) for value in unity_high],
...
def manifest_for(asset: AssetBuild, report: dict, signature: str) -> dict:
    return {
        ...
        "fire_contract": {
            "layers": ["FIX_Fire.Flame.Back", "FIX_Fire.Flame.Front"],
            "tongues_per_layer": [3, 4],
            "sections_per_tongue": 8,
            "radial_vertices_per_section": 8,
            "uv0": "existing positive atlas Fire sheet",
            "uv1": "u across tongue; v rooted base 0 to curved tip 1",
            "vertex_color": "R tongue phase; G heat variation; B back 0/front 1; A 1",
            "maximum_tip_axis_displacement_m": .04,
            "firebox_rear_plane_z": 3.835,
            "light_unity": list(ANCHORS_UNITY["ANCHOR_FireLight"]),
            "light_position_envelope_m": .04,
            "shared_shader": "BarPromenade/MothersHouseFlame",
        },
        "design_id": DESIGN_ID,
        ...
        "upper_storey_m": {
            ...
            "furnished": True,
            "rooms": {
                "bathroom": {"use": "shared_bathroom", "state": "lived_in",
                             "bounds_x": list(BATHROOM_X), "bounds_z": list(BATHROOM_Z),
                             "door_center_x": BATHROOM_DOOR_X, "door_center_z": BATHROOM_PARTITION_Z,
                             "door_width": BATHROOM_DOOR_WIDTH, "interactive": False},
                "north": {
                    "use": "parents_bedroom",
                    "state": "lived_in",
                    "bed": "double",
            ...
            "ceiling_lamps": {
                "corridor": list(ANCHORS_UNITY["ANCHOR_UpperCorridorLampLight"]),
                "north": [
        ...
        "wing_extension_m": {"x_min": -5.0, "x_max": UPPER_PARTITION_X,
                             "original_rear": 4.0, "rear_wall_plane": WING_REAR_WALL, "depth": .9},
...
def render_preview(asset: AssetBuild, path: Path) -> None:
    ...
    # Preview-only thermal colours use the same local fields as the Unity
    # shader. They are added after FBX export, never as another game atlas.
    for part in asset.parts:
        if part.role in {"firebox", "fire_logs", "fire_ash", "fire_embers"}:
            material = part.obj.data.materials[0].copy()
            material.name = "PREVIEW_" + part.name
            shader = material.node_tree.nodes.get("Principled BSDF")
            color = tuple(PREVIEW_COLORS[part.sheet][i] * part.tint[i] for i in range(3)) + (1.0,)
            shader.inputs["Base Color"].default_value = color
            if part.emissive:
                shader.inputs["Base Color"].default_value = (0, 0, 0, 1)
                shader.inputs["Emission Color"].default_value = part.tint
                shader.inputs["Emission Strength"].default_value = 2.4
            part.obj.data.materials[0] = material
        if part.role != "fire_flame":
            continue
        field = part.obj.data.color_attributes.new(name="PreviewThermal", type="FLOAT_COLOR", domain="POINT")
        for index, (u, v) in enumerate(part.flame_uv):
            heat = max(0.0, min(1.0, (1.0 - abs(u * 2 - 1) * .9) * (1.04 - v * .86)))
            body = min(1.0, heat * 1.35)
            core = max(0.0, min(1.0, (heat - .59) / .36))
            color = tuple(((1.65, .34, .028)[i] * (1 - body) +
                           (3.0, 1.37, .18)[i] * body) * (1 - core) +
                          (4.5, 3.25, 1.28)[i] * core for i in range(3))
            edge = max(0.0, min(1.0, (1.0 - abs(u * 2 - 1)) / .5))
            alpha = .84 * edge * min(1.0, v / .075) * min(1.0, (1 - v) / .18)
            field.data[index].color = (*color, alpha)
        material = bpy.data.materials.new("PREVIEW_" + part.name)
        material.use_nodes = True
        material.surface_render_method = "DITHERED"
        material.use_backface_culling = True
        shader = material.node_tree.nodes.get("Principled BSDF")
        vertex = material.node_tree.nodes.new("ShaderNodeVertexColor")
        vertex.layer_name = "PreviewThermal"
        shader.inputs["Base Color"].default_value = (0, 0, 0, 1)
        material.node_tree.links.new(vertex.outputs["Color"], shader.inputs["Emission Color"])
        material.node_tree.links.new(vertex.outputs["Alpha"], shader.inputs["Alpha"])
        shader.inputs["Emission Strength"].default_value = 1.0
        part.obj.data.materials[0] = material
    # The passive FBX was already exported. These two preview-only material
    # copies show the warm emission the runtime assigns from the part flags.
    for part in asset.parts:
        if part.name not in {"DRESS_UpperCorridor.LampGlass", "DRESS_UpperCorridor.LampBulb"}:
            continue
        preview_material = part.obj.data.materials[0].copy()
        preview_material.name = "PREVIEW_" + part.name
        shader = preview_material.node_tree.nodes.get("Principled BSDF")
        shader.inputs["Base Color"].default_value = part.tint
        shader.inputs["Emission Color"].default_value = part.tint
        shader.inputs["Emission Strength"].default_value = 1.8
        part.obj.data.materials[0] = preview_material
    ...
    def add_light(
        ...
        if name == "PreviewUpperCorridorLamp":
    ...
    add_light("PreviewUpperCorridorLamp", "POINT",
              ANCHORS_UNITY["ANCHOR_UpperCorridorLampLight"],
              145.0, (1.0, .76, .48), .08)
    ...
    camera.location = source_point((1.22, 1.10, 1.72))
    target = Vector(source_point((0.0, .78, 3.60)))
    camera.rotation_euler = (target - camera.location).to_track_quat("-Z", "Y").to_euler()
    camera_data.lens = 42.0
    scene.render.filepath = str(path.with_name(path.stem + "-Fireplace.png"))
    bpy.ops.render.render(write_still=True)
    camera.location = source_point(ANCHORS_UNITY["ANCHOR_BathroomCamera"])
    target = Vector(source_point(ANCHORS_UNITY["ANCHOR_BathroomCameraTarget"]))
    camera.rotation_euler = (target - camera.location).to_track_quat("-Z", "Y").to_euler()
    camera_data.lens = 20.3  # 58-degree vertical FOV at the 1024x640 preview aspect.
    hidden = [part.obj for part in asset.parts if part.name in {
        "FIX_BathroomSouthWall", "FIX_BathroomDoor.Frame"}]
    for obj in hidden:
        obj.hide_render = True
    add_light("PreviewBathroomLamp", "POINT", ANCHORS_UNITY["ANCHOR_BathroomLampLight"],
              95.0, (1.0, .76, .48), .12)
    add_light("PreviewBathroomWindow", "POINT", ANCHORS_UNITY["ANCHOR_BathroomColdWindowLight"],
              25.0, (.53, .68, .82), .3)
    scene.render.filepath = str(path.with_name(path.stem + "-Bathroom.png"))
    bpy.ops.render.render(write_still=True)
    for obj in hidden:
        obj.hide_render = False
    camera.location = source_point((-4.60, 5.70, -3.60))
    target = Vector(source_point((-2.80, 3.70, .20)))
    camera.rotation_euler = (target - camera.location).to_track_quat("-Z", "Y").to_euler()
    camera_data.lens = 20.3
    scene.render.filepath = str(path.with_name(path.stem + "-UpperCorridor.png"))
    bpy.ops.render.render(write_still=True)
    camera.location = source_point(ANCHORS_UNITY["ANCHOR_Camera"])
    target = Vector(source_point(ANCHORS_UNITY["ANCHOR_CameraTarget"]))
    camera.rotation_euler = (target - camera.location).to_track_quat("-Z", "Y").to_euler()
...
def main() -> int:
    ...
    # Blender's render/save operators interpret a Windows relative path from
    # the drive root; resolve overrides once before any Python or Blender IO.
    for field_name in ("blend", "fbx", "manifest", "preview"):
        path = getattr(args, field_name)
        setattr(args, field_name, path if path.is_absolute() else ROOT / path)
    asset = build()
    ...
    if args.validate_only:
        ...
        if args.manifest.is_file():
            ...
            if recorded.get("build_signature") != signature:
                raise SystemExit("The rebuilt model does not match its recorded deterministic signature")
        print(
    ...
    runtime_windows = window_runtime_source()
    if (not WINDOW_RUNTIME_PATH.is_file() or
            WINDOW_RUNTIME_PATH.read_text(encoding="utf-8") != runtime_windows):
        WINDOW_RUNTIME_PATH.write_text(runtime_windows, encoding="utf-8", newline="\n")
